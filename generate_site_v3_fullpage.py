...
for folder in sorted(base_dir.iterdir()):
    ...
    for i, img in enumerate(image_files):
        ...
        html_img = image_page_template.format(
            ...
            description=f"High quality image in {category} category",
            keywords=generate_meta_keywords(keywords, 8),
            css=category_css,
            img_src=img_src,
            alt=alt_text,
    ...
    description = f"Explore {category} style images: {meta_keywords}"
    title = f"{category.capitalize()} - Realistic Gallery"

    html_cat = category_page_template.format(
        title=title,
